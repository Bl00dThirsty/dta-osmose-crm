...
const EditInventory = () => {
  ...
  const { data: inventory } = useGetInventoryIdQuery(id);
  ...
  const [performedById, setPerformedById] = useState<number>();
  ...
  useEffect(() => {
    if (inventory) {
      setTitre(inventory.titre || "");
      setLocation(inventory.location || "");
      setNote(inventory.note || "");
      setPerformedById(inventory.performedById);
      setInventoryItems(inventory.inventoryItems.map((item: any) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/${institution}/inventory/${id}`);
    } catch (error) {
      console.error(error);
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Modifier l'inventaire</CardTitle></CardHeader>
      <CardContent className="space-y-4">
              ...
              return (
                <tr key={item.productId}>
                  ...
                  <td className="border px-2 py-1 text-center">
                    <Input type="number" value={item.countedQty} onChange={(e) => handleQtyChange(realIndex, Number(e.target.value))} />
                  </td>
                  <td className="border px-2 py-1 text-center">
                    {item.countedQty > 0 && Math.abs(item.countedQty - item.systemQty)}
                  </td>
                  <td className="border px-2 py-1">
                    <Input value={item.comment} onChange={(e) => handleCommentChange(realIndex, e.target.value)} />
                  </td>
                </tr>
              );
            })}
        ...
        <div className="flex justify-center mt-4 space-x-2">
          <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className="px-3 py-1 bg-blue-500 text-white rounded disabled:opacity-50">← Précédent</button>
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`px-3 py-1 mx-1 rounded ${currentPage === i + 1 ? 'bg-blue-600 text-white' : 'bg-white border'}`}>{i + 1}</button>
          ))}
          <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(currentPage + 1)} className="px-3 py-1 bg-blue-500 text-white rounded disabled:opacity-50">Suivant →</button>
        </div>
      </CardContent>
      <CardFooter>
        <Button onClick={handleSubmit}>Mettre à jour l’inventaire</Button>
      </CardFooter>
    </Card>
  );
};
